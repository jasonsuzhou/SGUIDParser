...
public class XMLUtil {
	...
	public static Element findNodeByAttributeValue(String attributeName, String attributeValue, Document doc) {
		Element node = doc.getRootElement();
		return parse(node, attributeName, attributeValue);
	}
	
	public static Element parse(Element node , String type , String val) {
		Element result = null;
		List<Element> allChildNodes = XMLUtil.findAllChildNodes(node);
		if (allChildNodes != null && !allChildNodes.isEmpty()) {
			for (Element ele : allChildNodes) {
				Attribute name = ele.attribute(type);
				if (name != null) {
	...
	public static void writeDocumentToFile(Document document, File targetFile) {
